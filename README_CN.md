...
#### 方式二：直接URL爬取流程
1. **准备URL文件**：在 `url.txt` 中添加项目URL，每行一个
2. **启动脚本**：运行 `python scraper2.py`
3. **浏览器打开**：脚本自动打开 Chrome 浏览器并访问 CNKI 网站
4. **手动登录**：在浏览器中点击"IP 登录"
5. **开始爬取**：在终端按 `Enter` 键，脚本开始自动爬取
6. **自动化流程**：
   - 逐个读取 `url.txt` 中的项目链接
   - 在新标签页打开每个URL
   - 提取详情页详细信息
   - 关闭详情页
7. **保存数据**：爬取完成后，数据自动保存为 Excel 文件
